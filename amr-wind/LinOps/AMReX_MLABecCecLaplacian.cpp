...
void MLABecCecLaplacian::setSchemeOrder(Real scheme_order) noexcept
{
    m_scheme_order = scheme_order;
}
    ...
    int amrlev, int mglev, MultiFab& out, const MultiFab& in) const
{
    ...
    const Real scheme_order = m_scheme_order;
        ...
        if (m_overset_mask[amrlev][mglev]) {
            ...
            AMREX_LAUNCH_HOST_DEVICE_LAMBDA(bx, tbx, {
                mlabecceclap_adotx(
                    ...
                    cscalar, ncomp, gradient_relax_factor, scheme_order);
            });
        }
    }
}
