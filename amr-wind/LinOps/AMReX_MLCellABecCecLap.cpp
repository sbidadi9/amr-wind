    ...
    for (int mglev = 1; mglev <= a_info.max_coarsening_level; ++mglev) {
        ...
        if (dom.coarsenable(2) && fine.boxArray().coarsenable(2)) {
            ...
            auto crse = std::make_unique<iMultiFab>(
                amrex::coarsen(fine.boxArray(), 2), fine.DistributionMap(), 1,
                1);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    LPInfo linfo = a_info;
    linfo.max_coarsening_level = std::min(
    ...
    for (amrlev = 1; amrlev < m_num_amr_levels; ++amrlev) {
        for (int mglev = 1; mglev < m_num_mg_levels[amrlev];
            ...
            if (Gpu::inLaunchRegion() &&
                ...
                auto const& finema =
                    m_overset_mask[amrlev][mglev - 1]->const_arrays();
                ParallelFor(
                    *m_overset_mask[amrlev][mglev],
                    [=] AMREX_GPU_DEVICE(
                        int box_no, int i, int j, int k) noexcept {
                        coarsen_overset_mask(
                            i, j, k, crsema[box_no], finema[box_no]);
                    });
...
#endif
                for (MFIter mfi(
                         *m_overset_mask[amrlev][mglev], TilingIfNotGPU());
                     mfi.isValid(); ++mfi) {
                    const Box& bx = mfi.tilebox();
                    Array4<int> const& cmsk =
                        m_overset_mask[amrlev][mglev]->array(mfi);
...
void MLCellABecCecLap::prepareForSolve() { MLCellLinOp::prepareForSolve(); }

void MLCellABecCecLap::update()
{
    if (MLCellLinOp::needsUpdate()) MLCellLinOp::update();
}

void MLCellABecCecLap::getFluxes(
...
void MLCellABecCecLap::applyInhomogNeumannTerm(int amrlev, MultiFab& rhs) const
{
    bool has_inhomog_neumann = hasInhomogNeumannBC();
    bool has_robin = hasRobinBC();

    if (has_robin) amrex::Abort("Robin not implemented for MLCellABecCecLap");
    ...
    Array<MultiFab const*, AMREX_SPACEDIM> const& bcoef =
        getBCoeffs(amrlev, mglev);

    // eta and C coeffs
    const Real eta = getCScalar();
    Array<MultiFab const*, AMREX_SPACEDIM> const& ccoef =
        getCCoeffs(amrlev, mglev);

    FArrayBox foo(Box(IntVect(0), IntVect(1)));

    bool has_bcoef = (bcoef[0] != nullptr);
    bool has_ccoef = (ccoef[0] != nullptr);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(rhs, mfi_info); mfi.isValid(); ++mfi) {
        ...
        for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
            Array4<Real const> const bfab =
                (has_bcoef) ? bcoef[idim]->const_array(mfi) : foo.const_array();

            Array4<Real const> const cfab =
                (has_ccoef) ? ccoef[idim]->const_array(mfi) : foo.const_array();
            ...
            for (int icomp = 0; icomp < ncomp; ++icomp) {
                ...
                if (m_lobc_orig[icomp][idim] == LinOpBCType::inhomogNeumann &&
                    outside_domain_lo) {
                    if (idim == 0) {
                        Real fac_vis = beta * dxinv[0];
                        Real fac_conv = 0.5 * eta;
                        AMREX_HOST_DEVICE_FOR_3D(blo, i, j, k, {
                            mllinop_apply_innu_xlo_ic(
                                i, j, k, rhsfab, mlo, bfab, cfab, bctlo, bcllo,
                                bvlo, fac_vis, fac_conv, has_bcoef, has_ccoef,
                                icomp);
                        });
                    } else if (idim == 1) {
                        Real fac_vis = beta * dxinv[1];
                        Real fac_conv = 0.5 * eta;
                        AMREX_HOST_DEVICE_FOR_3D(blo, i, j, k, {
                            mllinop_apply_innu_ylo_ic(
                                i, j, k, rhsfab, mlo, bfab, cfab, bctlo, bcllo,
                                bvlo, fac_vis, fac_conv, has_bcoef, has_ccoef,
                                icomp);
                        });
                    } else {
                        Real fac_vis = beta * dxinv[2];
                        Real fac_conv = 0.5 * eta;
                        AMREX_HOST_DEVICE_FOR_3D(blo, i, j, k, {
                            mllinop_apply_innu_zlo_ic(
                                i, j, k, rhsfab, mlo, bfab, cfab, bctlo, bcllo,
                                bvlo, fac_vis, fac_conv, has_bcoef, has_ccoef,
                                icomp);
                        });
                ...
                if (m_hibc_orig[icomp][idim] == LinOpBCType::inhomogNeumann &&
                    outside_domain_hi) {
                    if (idim == 0) {
                        Real fac_vis = beta * dxinv[0];
                        Real fac_conv = 0.5 * eta;
                        AMREX_HOST_DEVICE_FOR_3D(bhi, i, j, k, {
                            mllinop_apply_innu_xhi_ic(
                                i, j, k, rhsfab, mhi, bfab, cfab, bcthi, bclhi,
                                bvhi, fac_vis, fac_conv, has_bcoef, has_ccoef,
                                icomp);
                        });
                    } else if (idim == 1) {
                        Real fac_vis = beta * dxinv[1];
...
void MLCellABecCecLap::applyOverset(int amrlev, MultiFab& rhs) const
{
...
#ifdef AMREX_USE_PETSC
std::unique_ptr<PETScABecLap> MLCellABecCecLap::makePETSc() const
{
    const BoxArray& ba = m_grids[0].back();
    const DistributionMapping& dm = m_dmap[0].back();
    const Geometry& geom = m_geom[0].back();
